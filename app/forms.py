...
class RegistrationForm(FlaskForm):
    username = StringField('Username', validators=[DataRequired(), Regexp(
        '^[a-zA-Z0-9_.]*$', message='Username must not contain spaces')])
    full_name = StringField('Full Name', validators=[DataRequired(), Regexp(
        '^[a-zA-Z ]*$', message='Your full name must not contain special characters')])
...
class EditProfileForm(FlaskForm):
    ...
    about_me = TextAreaField('About me', validators=[
                             Length(min=0, max=50)], widget=TextArea())
...
class PostForm(FlaskForm):
    title = TextAreaField('Title', validators=[
                          DataRequired(), Length(min=0, max=40)], widget=TextArea())
    body = TextAreaField('Body', validators=[DataRequired(), Length(
        min=0, max=8000, message="Blog post too long")], widget=TextArea())
